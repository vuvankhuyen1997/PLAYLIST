...
def write_playlist_txt(playlist):
	...
	print("Succesfully write playlist to txt")
...
def show_menu():
	...
	print("| Option 7: Exit and Save   |")
	print(" --------------------------")

def select_in_range(prompt, min, max):
	choice = input(prompt)
	while not choice.isdigit() or min > int(choice) or int(choice) > max :
...
def play_video(playlist):
	...
	print("\nTotal videos: ", total)
	print_videos(playlist.videos)
	choice = select_in_range("\nSelect a video (1 -> " + str(total) + "):", 1, total)
