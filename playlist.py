...
class TextButton:
	...
	def draw(self):
		...
		if self.is_mouse_on_text():
			...
			pygame.draw.line(screen, (0,0,255), (self.position[0], self.position[1]+self.text_box[3]), (self.text_box[2]+self.position[0], self.position[1]+self.text_box[3]))
		else:
			text_render = font.render(self.text, True, (0,0,0))
				...
				for i in range(len(playlists_btn_list)):
					if playlists_btn_list[i].is_mouse_on_text():
						...
						for j in range(len(playlist.videos)):
							video_btn = TextButton(str(j+1) + ". " +playlist.videos[j].title.rstrip(), (250,50+margin*j))
